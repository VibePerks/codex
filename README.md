...
```
./uninstall.sh          # macOS/Linux: remove the shell integration
./uninstall.ps1         # Windows PowerShell
```

This removes the VibePerks block from your shell rc so the sponsor line stops
showing. Also remove the Codex plugin hooks through Codex's own plugin system. Your
local token/cache lives in `~/.vibeperks/` - delete that folder if you want to remove
it too.

## Build

Installing needs **no Go and no manual build**: `bin/vibeperks-codex` is a committed
launcher that downloads the prebuilt binary for your platform from the GitHub Release on
first use and caches it (the Windows installer downloads it directly). Building from
source is only for development (Go 1.23+):
